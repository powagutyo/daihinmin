...
public class MeldData implements Cloneable {
	...
	private int win = 0;
	/** 出すカード群 **/
	private ArrayList<Integer> arrayCards = new ArrayList<Integer>(11);
	/** カードの枚数 **/
	private int size = 0;
	/** PASSかどうか **/
	...
	public MeldData(MeldData md) {
		this.n = md.getN();
		this.win = md.getWin();
		this.arrayCards = (ArrayList<Integer>) md.getArrayCards().clone();
	...
	public void upDateChild(int point, ArrayList<Integer> orderT, int depth,
		...
		double logn = Math.sqrt(2 * (Math.log(visitNum) / Math.log(2)));
	...
	public ArrayList<Integer> getArrayCards() {
		return arrayCards;
	}
	...
	public int getArrayCards(int num) {
		return arrayCards.get(num);
	}
