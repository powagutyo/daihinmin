...
public class UCTPlayer extends BotSkeleton {
	...
	/** 全員の手札群 **/
	ArrayList<ArrayList<Card>> playersCard = new ArrayList<ArrayList<Card>>(players);
	...
	public void played(java.lang.Integer number, Meld playedMeld) {
		...
		if (playedMeld != null) {
			...
			if (playedMeld != PASS) {
				...
				//mh.setPlayerCards(playedMeld, playersInformation().getSeatOfPlayer(number), renew);

				mh.dealingPutCards(playedMeld,playersInformation().getSeatOfPlayer(number), fieldData, myData.getSeat());
				//TODO 大富豪、大貧民などの処理
				if (!firstGame) {
					Cards cards = Cards.EMPTY_CARDS;
	...
	public void init() {
		...
		mh.initHands(myData, fieldData); // 手札を生成
	...
	public void firstUpdate() {
		...
		if (exchangeCard && false) { // カード交換を行った時に処理
			exchangeCard = false;

			//mh.updateToExchange(handOverCard, playersInformation().getSeatOfPlayer(exchangedPlayer));// 交換したカードをスワップさせる
		}
