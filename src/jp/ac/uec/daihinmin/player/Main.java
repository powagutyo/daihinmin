...
import tactics.T_Joker;
import tactics.T_Pass;
...
public final class Main extends BotSkeleton {
	...
	/** 通常行う戦略 */
	private Tactics[] tactics = { new T_Joker(), new T_Sp3(), new T_Single(),
			new T_Single_Reverse(), new T_Group(), new T_Group_reverse(),
			new T_Sequence(), new T_Sequence_Reverse(), new T_Pass() };
	/** 新しく自分の番が来た時の戦略 */
	private Tactics[] tactics_renew = { new T_Joker(), new T_Renew(),
			new T_Renew_Reverse() };
	...
	public Cards requestingGivingCards() {
		...
		try {
			...
			myState.setMyRank(myRank);// 自分のランクを登録
	...
	public void played(java.lang.Integer number, Meld playedMeld) {
		...
		try {
			...
			if (playedMeld != PASS) {
				//pd.discardPlayersHand(number, playedMeld.asCards().size());
			}
			myState.look_Card(number, playedMeld, this);// 捨てたカードを記憶させる
			myState.searchStrongCard(myState.isReverse());// 出ていないカードで一番強いカードを検索
	...
	public void playAccepted(java.lang.Integer number, Meld playedMeld) {
		...
		try {
			...
			//myState.calcAverage(this.hand());// 自分の手札と相手の手札を見て平均値を出す
	...
	/**
	 * useTacticeメソッドで行うMystateのデータの更新を行うメソッド
	 */
	public void updateState() {

		myState.calcAverage(this.hand());// 自分の手札と相手の手札を見て平均値を出す

		myState.haveJoker(this.hand());// Jokerの有無を調べる

		myState.examineLapPair(this.hand());// 階段とペア出しが全て重なっているかどうかを調べる

		myState.canReverse(this.hand());// 革命を起こせるかどうかの判定

		myState.doOrNotReverse();// 革命を起こすか否か
	...
	public Meld useTactics() {
		PlayersInformation pi = this.playersInformation();
		int[] numCardsOfPlayers = pi.numCardsOfPlayers();

		Meld finalMeld = null;// 最終的に決まった役

		// モンテカルロ実行部

		// mc.init(this, myState, pd);

		// mc.search(this);

		try {
			updateState();// myDateの更新

			if (myState.getTurn() > 1000) {// ありえないターン数の時
