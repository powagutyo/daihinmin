...
public class DaihinminStrategy extends Strategy {
	@Override
	...
	public Meld execute(BotSkeleton bot, State state) {
		if(state.order && state.numberOfPlayers() >= 3 && state.hand.size() > 8) {
			return null;
		}
		...
		if(state.order && state.hand.size() > 10 && size > 0 && state.numberOfMyStrongerCards(cards.get(0)) < size + 1) {
			return null;
