...
public class HeiminStrategy extends Strategy {
	@Override
	public boolean satisfyCondition(BotSkeleton bot, State state) {
		if(state.botRank == 3) {
			...
			return strongerCards * 2.3 > bot.handSize();
		} else {
			return false;
		}
	}
	...
	public Meld execute(BotSkeleton bot, State state) {
		if(state.order && state.numberOfPlayers() == 5 && state.hand.size() > 8) {
			return null;
		}
		...
		if(state.order && state.hand.size() > 7 && size > 1 && state.numberOfMyStrongerCards(cards.get(0)) < size + 1) {
			return null;
