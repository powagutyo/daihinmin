...
/**
 * 相手の手に対する一枚出しの戦略(革命バージョン)
 */
public class T_Single_Reverse extends Tactics {
	...
	public Meld discardMeld(Melds melds, MyState state, BotSkeleton bs) {
		...
		if(cards.size() ==2 && state.isJoker()){//自分の手札が2枚でかつjokerを含む時
			...
			finalMelds = finalMelds.extract(Melds.rankOf(Rank.JOKER_LOWEST));
		...
		melds = melds.extract(Melds.rankOver(Rank.EIGHT));
		...
		if (state.isDoReverse()) {// 革命を行う時
			...
			for (Meld meld : state.getReverseMelds()) {
				...
				if (lastMeld.rank().toInt() >= 9 && state.isExistEight()) {
		...
		if (state.getSituation() == Situation.Opening
			...
			melds = melds.extract(Melds.rankOver(strongCard.rank()));// 一番強いのカードを温存する
		}
		
		if (state.getThreeAndOver() < 2) {
			/*自分が平民以下の時に縛りを意識させる*/
			...
			myMelds = myMelds.extract(Melds.rankOf(Rank.THREE));// 3の数のみ抜粋
		...
		if (state.isExistEight() && lastMeld.rank().toInt() > 8) {
			if (state.getStrongSingleMelds().size() >= 2 || week >= 4) {
		...
		melds = melds.extract(Melds.rankUnder(lastMeld.rank()));

		if (melds.size() != 0) {
			int size = melds.size() - 1;
			if (melds.get(size).rank() == Rank.EIGHT
					&& state.getSituation() != Situation.Last) {
				if (melds.size() != 1)
					finalMeld = melds.get(size - 1);
			} else {
				finalMeld = melds.get(size);
			}
		}
